...
const PatientEdit = () => {
  ...
  const burnCauses = [
    "Haşlanma", "Flaş", "Temas", "Alev", "Kimyasal","Radrasyon",
    "Elektrik-Arc", "Elektrik-Yüksek Voltaj","Elektrik-Düşük Voltaj", "Diğer"
  ];
  const burnAreas = [ 
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${BACKEND_API}/api/Patient/${id}`);
        if (!response.ok) {
